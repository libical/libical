...
void strip_errors(vcardcomponent *comp)
{
    ...
    for (prop = vcardcomponent_get_first_property(comp, VCARD_XLICERROR_PROPERTY); prop; prop = next) {
...
static vcardcomponent *test_comp_vanew(void)
{
    ...
    card = vcardcomponent_vanew(VCARD_VCARD_COMPONENT, vcardproperty_new_version(VCARD_VERSION_40),
                                vcardproperty_new_kind(VCARD_KIND_INDIVIDUAL), (void *)0);
...
static void test_add_props(vcardcomponent *card)
{
    ...
    vcardproperty *prop = vcardproperty_vanew_note("Test vCard", vcardparameter_new_language("en"),
                                                   vcardparameter_new_pid(sa), (void *)0);
    ...
    prop =
        vcardproperty_vanew_logo("https://example.com/logo.png", vcardparameter_new_mediatype("image/png"), (void *)0);
    vcardcomponent_add_property(card, prop);

    /* Create and add UID property */
    prop = vcardproperty_vanew_uid("foo-bar", vcardparameter_new_value(VCARD_VALUE_TEXT), (void *)0);
    vcardcomponent_add_property(card, prop);

    /* Create and add TEL property */
    prop = vcardproperty_vanew_tel("tel:+1-888-555-1212", vcardparameter_new_value(VCARD_VALUE_URI), (void *)0);
    vcardcomponent_add_property(card, prop);

    /* Create and add LANG properties */
    prop = vcardproperty_vanew_lang("fr", vcardparameter_new_pref(2), (void *)0);
    vcardcomponent_add_property(card, prop);

    prop = vcardproperty_vanew_lang("en", vcardparameter_new_pref(1), (void *)0);
    vcardcomponent_add_property(card, prop);
